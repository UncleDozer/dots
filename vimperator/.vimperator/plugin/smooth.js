let self = liberator.plugins.smooziee = (function(){
    ...
    var scroll_amount = liberator.globalVariables.smooth_scroll_amount || '150';
    ...
    mappings.addUserMap(
        ...
        "Smooth scroll down",
        function(count){
            self.smoothScrollBy(getScrollAmount((screen.height / 3) * (count || 1)));
        },
        {
            count: true
        }
    ...
    mappings.addUserMap(
        ...
        "Smooth scroll up",
        function(count){
            self.smoothScrollBy(getScrollAmount((screen.height / 3) * (count || 1)) * -1);
        },
        {
            count: true
        }
    ...
    mappings.addUserMap(
        ...
        "Smooth scroll down",
        function(count){
            self.smoothScrollBy(getScrollAmount((buffer.pageHeight) * (count || 1)));
        },
        {
            count: true
        }
    ...
    mappings.addUserMap(
        ...
        "Smooth scroll up",
        function(count){
            self.smoothScrollBy(getScrollAmount((buffer.pageHeight) * (count || 1)) * -1);
